...
, REG.Top_Cities
, case 
		when CT.FiOS is not null and CT.RCN is not null  then 'FiOS&RCN'
		when CT.FiOS is not null then 'FiOS'
		when CT.[U-verse] is not null and HDS.region <> 'KEY' then 'Frontier'
		when CT.RCN is not null and CMT.Fiber <> 'Fiber' then 'RCN'
		when CMT.Fiber = 'Fiber' and HDS.Region <> 'WNE' then 'FiOS'
		when CMT.Fiber = 'Fiber' and HDS.Region = 'WNE' then 'Frontier'
		when HDS.Region = 'WNE' and CT.[Other] in ('VTEL') then CT.[Other]
		when HDS.Region = 'blt' and CT.[Other] in ('Lumos Networks','nDanville') then CT.[Other]
		when HDS.Region = 'fre' and CT.[Other] in ('Cablevision','Service Electric Cable') then CT.[Other]
		when HDS.Region = 'gbr' and CT.[Other] in ('TDS Fiber') then CT.[Other]
		when HDS.Region = 'key' and CT.[Other] in ('Blue Ridge','Zito Media','KCI','Service Electric Cable','Atlantic Broadband') then CT.[Other]
		when HDS.Region = 'wne' and CT.[Other] in ('TVC','BT') then CT.[Other]
		when CT.Other is not null then 'Other'
		else 'No Overbuilder'
		end as Competitor
...
left join Competitive.dbo.competitive_threat CT
	on CMT.fullfipsid = CT.CensusBlock
GROUP BY HDS.FME
, HDS.Region
, REG.DMA_NAME
